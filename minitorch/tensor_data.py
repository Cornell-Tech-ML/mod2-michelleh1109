...
class TensorData:
    ...
    def get(self, key: UserIndex) -> float:
        """Retrieve the value at a specific multi-dimensional index in the tensor."""
        x: float = self._storage[self.index(key)]
        return x

    def set(self, key: UserIndex, val: float) -> None:
        """Sets value at a specific index in the tensor"""
    ...
    def permute(self, *order: int) -> TensorData:
        ...
        # Reorder the shape according to the given order
        new_shape = tuple(self.shape[dim] for dim in order)

        # Reorder the strides according to the given order
        new_strides = tuple(self.strides[dim] for dim in order)

        # Return a new TensorData object with permuted shape and strides
